...
## [Unreleased]
### Добавлено
- Метод `__validate_bucket_name` в `S3Provider` для валидации имени бакета; 
- Метод `_create_bucket` в `S3Provider` для идемпотентного создания бакета; 
- Метод `_delete_bucket` в `S3Provider` для идемпотентного удаления бакета;
- Юнит-тест для `__validate_bucket_name` с проверкой работоспособности;
- Юнит-тесты для метода `_create_bucket` с проверкой различных сценариев работоспособности;
- Юнит-тесты для метода `_delete_bucket` с проверкой различных сценариев работоспособности;
...
### Изменено
- Класс тестов `TestSkipIfNone` под обновленный функционал;
- Декоратор `skip_if_none` теперь принимает значение любого типа;
- Импорт переменной окружения harbor при запуске pipeline;


## [0.4.3]

### Добавлено
- Плагин `flake8-tidy-imports` с правилом `ban-relative-imports = true`
- Модуль `base`, `database` с базовым классом сервис слоя и классом настроек подключения к бд;
- Тесты `test_base`, `test_database` базового класса сервис слоя и класса настроек подключения к бд;


### Изменено
- Заменен относительный импорт на абсолютный для `BaseRepositoryClass`


## [0.4.2]

### Добавлено
- Модуль `base` с базовым классом сервис слоя;
- Тесты `test_base` базового класса сервис слоя;
